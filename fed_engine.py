...
class S2SProtocolEngine:
    ...
    def build_hello(self, active_channels: list[str]) -> bytes:
        payload = {0: 1, 1: int(self.bandwidth_class.value), 2: active_channels}
    ...
    def process_inbound_frame(self, raw_bytes: bytes) -> tuple[Opcode, list[bytes]]:
        try:
        ...
        if opcode == Opcode.FED_HELLO:
            ...
            if self.db.add_channel(str(chan_name), str(desc)):
            ...
            if pub_key:
                try:
                    reconstructed = RNS.Identity.from_bytes(pub_key)
                    if reconstructed and reconstructed.hash == origin_hash:
            ...
            for msg_tuple in payload[0]:
                ...
                if msg_id_hex not in self.seen_msg_ids:
                    # Mark seen before verifying, on purpose: msg_id already
                    # encodes content+sender+timestamp+nonce, so a legitimate
                    # resend under the same msg_id with a *different*
                    # (corrected) signature can't happen -- this just stops a
                    # replayed bad frame from being re-verified (cheap, but
                    # not free) on every retransmission.
                    self.seen_msg_ids.add(msg_id_hex)
